import re
def extract_percentagesJP(text):
    pourcentageE=-1
    pourcentageI=-1
    keywords = ["judging", "Judging"]
    for keyword in keywords:
        ...
        if(pourcentageI != -1):
            break
    keywords = ["perceiving", "Perceiving"]
    for keyword in keywords:
        result = extract_context(text, keyword)
        for context in result:
            numbers = re.findall(r'\d+', context)
    ...
    if ((pourcentageE ==-1) and (pourcentageI==-1 )):
        ...
        if(pourcentageE > pourcentageI):
            ...
            pourcentageE = 100 - pourcentageI
    percentages = {'judging': None, 'perceiving': None}
    percentages['judging'] = pourcentageI
    percentages['perceiving'] = pourcentageE
    return percentages
...
def check_order1(text):
    extrovert_index = text.find("perceiving")
    introvert_index = text.find("judging")
...
def findper(text):
    ...
    extrovert_positions = []
    extrovert_position = text.find("perceiving")
    while extrovert_position != -1:
        extrovert_positions.append(extrovert_position)
        extrovert_position = text.find("perceiving", extrovert_position + 1)
    introvert_positions = []
    introvert_position = text.find("judging")
    while introvert_position != -1:
        introvert_positions.append(introvert_position)
        introvert_position = text.find("judging", introvert_position + 1)
