import re
def extract_percentagesEI(text):

    pourcentageE=-1
    pourcentageI=-1
    keywords = ["introvert", "introversion"]
    for keyword in keywords:
        result = extract_context(text, keyword)
        for context in result:
            if(keyword =="introvert"):
        ...
        if(pourcentageI != -1):
            break
    keywords = ["extrovert", "extroversion"]
    for keyword in keywords:
        result = extract_context(text, keyword)
        for context in result:
            numbers = re.findall(r'\d+', context)
    ...
    if ((pourcentageE ==-1) and (pourcentageI==-1 )):
        ...
        if(pourcentageE > pourcentageI):
            ...
            pourcentageE = 100 - pourcentageI
    percentages = {'introvert': None, 'extrovert': None}
    percentages['introvert'] = pourcentageI
    percentages['extrovert'] = pourcentageE
    return percentages
...
def check_order1(text):
    extrovert_index = text.find("extrovert")
    introvert_index = text.find("introvert")
...
def check_order2(text):
    extrovert_index = text.find("extroversion")
    introvert_index = text.find("introversion")
...
def findper(text):
    ...
    extrovert_positions = []
    extrovert_position = text.find("extrovert")
    while extrovert_position != -1:
        extrovert_positions.append(extrovert_position)
        extrovert_position = text.find("extrovert", extrovert_position + 1)
    introvert_positions = []
    introvert_position = text.find("introvert")
    while introvert_position != -1:
        introvert_positions.append(introvert_position)
        introvert_position = text.find("introvert", introvert_position + 1)
