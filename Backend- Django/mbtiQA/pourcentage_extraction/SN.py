import re
def extract_percentagesSN(text):
    pourcentageE=-1
    pourcentageI=-1
    keywords = ["Sensing", "sensing"]
    ...
    if ((pourcentageE ==-1) and (pourcentageI==-1 )):
        ...
        if(pourcentageE > pourcentageI):
            ...
            pourcentageE = 100 - pourcentageI
    percentages = {'sensing': None, 'intuition': None}
    percentages['sensing'] = pourcentageI
    percentages['intuition'] = pourcentageE
    return percentages
...
def check_order1(text):
    extrovert_index = text.find("intuition")
    introvert_index = text.find("sensing")
...
def findper(text):
    ...
    extrovert_positions = []
    extrovert_position = text.find("intuition")
    while extrovert_position != -1:
        extrovert_positions.append(extrovert_position)
        extrovert_position = text.find("intuition", extrovert_position + 1)
    introvert_positions = []
    introvert_position = text.find("sensing")
    while introvert_position != -1:
        introvert_positions.append(introvert_position)
        introvert_position = text.find("sensing", introvert_position + 1)
